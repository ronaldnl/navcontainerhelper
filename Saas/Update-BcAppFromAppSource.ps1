<#
 .Synopsis
  Function for updating an AppSource App in an online Business Central environment
 .Description
  Function for updating an AppSource App in an online Business Central environment
 ...
 .Example
  ...
  Update-BcAppFromAppSource -bcAuthContext $authContext -environment 'MySandbox' -AppId '55ba54a3-90c7-4d3f-bc73-68eaa51fd5f8' -appVersion 
#>
function Update-BcAppFromAppSource {
    ...
    try {
        ...
        if ($bcEnvironment.Type -eq 'Production' -and !$allowInstallationOnProduction) {
            throw "If you want to update an app in a production environment, you need to specify -allowInstallOnProduction"
        }
        ...
        if ($appExists) {
            ...
            try {
                $operation = Invoke-RestMethod -Method Post -UseBasicParsing -Uri "$($bcContainerHelperConfig.apiBaseUrl.TrimEnd('/'))/admin/$apiVersion/applications/BusinessCentral/environments/$environment/apps/$appId/update" -Headers $headers -ContentType "application/json" -Body ($body | ConvertTo-Json)
            }
            catch {
...
Export-ModuleMember -Function update-BcAppFromAppSource
